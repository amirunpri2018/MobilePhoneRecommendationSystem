...
//        System.out.println(loadFrom);
    ...
    public static ArrayList<String> readLine(String str) {
        ...
        /**
         * [^\"] token starting with something other than " and , \S* followed
         * by zero or more non-space characters | or \"\"\".+?\"\"\" a """
         * symbol followed by whatever until another """ Pattern.Multiline
         * Enables multiline reading Pattern.Dotall Ignores normal line ending
         */
...
//        System.out.println(Arrays.toString(s));
    ...
    public static ArrayList<MobilePhone> merge_sort(ArrayList<MobilePhone> arr) {  //merge sort algorithm
        if (arr.size() < 9)
            return bubble_sort(arr);    //use bubble sort once the arr gets small enough
        else
            return merge_arr(
                    merge_sort(new ArrayList<MobilePhone>(arr.subList(0, arr.size() / 2))), merge_sort(new ArrayList<MobilePhone>(arr.subList(arr.size() / 2, arr.size())))
            ); //recursion
    }
    ...
    private static ArrayList<MobilePhone> merge_arr(ArrayList<MobilePhone> a, ArrayList<MobilePhone> b) {   //merging arrays
        ...
        if (a.get(a.size() - 1).getRating() >= b.get(0).getRating()) {
            ...
            while (a.size() > 0 && b.size() > 0) {
                if (a.get(0).getRating() >= b.get(0).getRating())
                    answer.add(a.remove(0));
                else
                    answer.add(b.remove(0));
            }
            answer.addAll(a);
            answer.addAll(b);
        }
